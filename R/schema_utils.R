...
as_table_schema <- function(df,
  ...
  if(!"synapseclient.table.Schema" %in% class(schema) && is_valid_syn_id(schema)) schema <- synapser::synGet(schema)
  col_schema <- synapser::synGetTableColumns(schema) %>% synapser::as.list()
  ...
  for(i in seq_along(col_type)) {
    ...
    if(grepl("STRING", col_type[i])) {
      ...
      size_fail <- sapply(values, function(x) any(sapply(x, function(s) if(is.na(s)) FALSE else nchar(s) > maxsize)))
  ...
  table_data <- synapser::Table(schema, df)
  table_data
}
