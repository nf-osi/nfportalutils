...
annotate_with_manifest <- function(manifest, ignore_na = TRUE, ignore_blank = TRUE, verbose = FALSE) {
  ...
  for(entity in names(annotations)) {
    synapser::synSetAnnotations(entity = entity, annotations = as.list(annotations[[entity]]))
  }
...
copy_annotations <- function(entity_from,
  ...
  annotations <- synapser::synGetAnnotations(entity_from)
  if(is.null(select)) {
    cp <- annotations
  } else {
    cp <- reticulate::dict()
  ...
  if(update) {
    for(e in entity_to) {
      synapser::synSetAnnotations(e, annotations = cp)
    }
  } else {
    return(cp)
  }
