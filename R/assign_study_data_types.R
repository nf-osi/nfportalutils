...
#' @description Retrieve valid subclasses of a value in a JSON-LD schema. Notably, I know very little about JSON-LD, so this function is 1: very hacky, and 2: may not work on other JSON-LD files.
#' @param schema_url Default: the NF-OSI JSON-LD schema.
#' @param source_name_schema Default = DataType. The value for which you'd like to find the associated subclasses.
...
get_valid_values_from_json_schema <- function(schema_url = 'https://raw.githubusercontent.com/nf-osi/schematic/develop/data/schema_org_schemas/NF.jsonld',
                                              source_name_schema = "DataType"){

  json_dictionary <- jsonlite::fromJSON({{schema_url}})
  json_datatype <- json_dictionary$`@graph`[json_dictionary$`@graph`$`rdfs:subClassOf`==glue::glue('list(`@id` = "bts:{source_name_schema}")'),]$`sms:displayName`
