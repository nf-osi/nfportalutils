...
The code supporting this functionality heavily adapted some code originally written by [hhunterzinck](https://github.com/hhunterzinck).
...
- Defaults are for known NF-OSI processed data outputs. If these defaults don't apply because this was ad hoc processing or some variation in the SOP, it is recommended to take a look at the lower-level utils `make_meta_*` or understand how to edit the files manually.
...
Thus, `clinical_data` is usually prepared from an existing Synapse table. Most NF clinical data is in a single table -- `individualId`, `specimenID`, `age`, `sex`, `tumorType`, etc. all in one table because of the current annotation approach. 
The workflow handles creating a Patient file if some Patient-specific attributes are detected in the table. 
However, projects may have both Patient and Sample, which is fine. 
The examples below will show how to work with both options. 

Important to this step is a global export list (mapping) of clinical variables that clinical data export relies on. 
...
#### Export using both Patient and Sample clinical data table

```{r add_clinical_option_2, eval=FALSE}

# example queries when all patients/samples are releasable
clinical_data_sample <- "select * from syn1111111"
clinical_data_patient <- "select * from syn2222222"

cbp_add_clinical(clinical_data_sample, ref_map, type = "sample")
cbp_add_clinical(clinical_data_patient, ref_map, type = "patient")
