...
This vignette shows how to automatically generate metadata manifests and provenance information for data processed by nextflow workflows (nf-rnaseq and nf-sarek). These manifests can be validated with schematic and used to annotate files in Synapse or create [Synapse Datasets](https://help.synapse.org/docs/Datasets.2611281979.html).
...
result <- annotate_nf_workflow(
  publish_dir = "syn51476810",      # Top-level output directory (contains pipeline_info)
  fileview = "syn11601481",         # Fileview scoping the output files
...
- Discovers all available output types
- Generates properly annotated manifests for each output type

**Optional parameters** are available for:
...
# Inspect manifest before submission
...
Alternatively, validate with [schematic](https://schematic.api.sagebionetworks.org/v1/ui/) before submission.
...
add_activity_batch(
  output_id = sample_io$output_id,
  activity_name = sample_io$workflow,
  activity_link = wf_link,
  input_id = sample_io$input_id
)
```

Provenance establishes the computational lineage: raw data → workflow → processed data.

### 3. Create Synapse Dataset (optional)

Package the annotated files as a reusable dataset:

```{r create-dataset, eval=FALSE}
dataset_salmon <- new_dataset(
  name = "STAR Salmon Gene Expression Quantification",
  parent = "syn12345678",  # Your project or folder
...
add_activity_batch(
  output_id = sample_io$output_id,
  activity_name = sample_io$workflow,
  activity_link = wf_links[sample_io$workflow],
  input_id = sample_io$input_id
)
```
