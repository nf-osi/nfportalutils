...
if(outside_us) {
  items <- c(items, "Yes, data is outside US")
}

ext_gov <- dsp$governance$contributorDUO$DUOPX_details
if(!is.null(ext_gov) || ext_gov != "") {
  items <- c(items, paste("This project appears to have extended governance request:", htmltools::tags$em(ext_gov)))
}
...
if(length(items)) {
  htmltools::tagList(
    htmltools::div(
      htmltools::tags$ul(
        lapply(items, function(i) htmltools::tags$li(i))
      )
    )
  )
} else {
