...
if(data_not_expected) {
  cat("*Skipped comparison check because DSP indicates **Data Not Expected**.*")
} else if(!length(dsp$dataDeposit)) {
   cat("*Data deposit appears to not be filled out.*")
} else {
  expectation <- dsp$dataDeposit
  dsp_datasets <- setNames(paste0("D", seq_along(expectation)),  sapply(dsp$dataDeposit, `[[`, "dataLabel"))
  in_project <- list_project_datasets(project_id)
  project_datasets <- setNames(sapply(in_project, `[[`, "id"), sapply(in_project, `[[`, "name"))
}
